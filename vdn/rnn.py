...
# rnn_size = 64
# total_epochs = 20
# # x_train = tf.placeholder('float', shape=(100, 144, 256))
# # y_train = tf.placeholder('float', shape=())
...
# # inp("1.mp4")
# # take the mid 100 frames of the video
# # downscale to 256 x 144
# def extract_features():

# 	x = glob.glob('data/positive/*')[ :10]
# 	y = [[1, 0]] * len(x)*10

# 	tmp = glob.glob('data/negative/*')[:10]
# 	x = x + tmp
# 	y = y + [[0, 1]]* 10
# 	all_frames = []
...
# 		for i in range(0, tot_vid_frames):
# 			success, frame = vid.read()
# 			# print(frame, success)
...
# 	print("\nall frames :\n", all_frames)
# #	q = list(zip(all_frames, y))
# #	random.shuffle(q)
# #	all_frames, y = list(zip(*q))
# #	all_frames, y = list(all_frames), list(y)
# 	l = len(all_frames)

# 	x_train = all_frames[ : l - l//4]
# 	x_test = all_frames[l - l//4 : ]
...
def load_set(videofile):
    ...
    frames = []
    img = []
    counter = 0        ### create an array to save each image as an array as its loaded 
    while success: ### while success == True
        ...
        for j in range(0,99):
            try:
                ...
                # print ("sucesss",success," image",img)
                ...
                #print 'There are ', count, ' frame; delete last'        read_frames(videofile, name)
...
img_filepath = 'data/' #### the filepath for the training video set
neg_all = glob.glob(img_filepath + 'negative/*.mp4')[10:20]               #### negative examples - ACCV
pos_all = glob.glob(img_filepath + 'positive/*.mp4')[10:20]   
...
labels = numpy.concatenate(([1]*len(pos_all), [0]*len(neg_all)))  ### create the labels for the videos
...
x_testA = numpy.array(x_t1[len(x_t1)//2:]); y_testA = numpy.array(y_t1[len(y_t1)//2:])    #### test set

### valid set for model
x_testB = numpy.array(x_t1[:len(x_t1)//2]); y_testB = numpy.array(y_t1[:len(y_t1)//2])    ### need to be arrays
x_test = make_dataset(x_testB)
y_test = make_dataset(y_testB)
...
for i in range(0, 30):               ### number of epochs
    ...
    x_shuff = numpy.array(x_shuff); y_shuff=numpy.array(y_shuff) ### back into arrays
    ...
    for j,xb in enumerate(x_batch):  ### for each batch in the shuffled list for this epoch
        ...
        model.fit(xx, yy,                            ### fit training data
                  ...
                  validation_data=(x_test.reshape(2), y_test.reshape(2)),  ### validation set from up earlier in notebook
